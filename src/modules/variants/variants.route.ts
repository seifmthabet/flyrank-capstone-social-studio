...
export const createVariantsRoute = (deps: { variantsService: IVariantsService }) => {
    const variantsRouter = Router();

    variantsRouter.get("/:id", async (req: Request, res: Response) => {
        const variant = await deps.variantsService.findById(String(req.params.id));
    ...
    variantsRouter.post("/:id/schedule", async (req: Request, res: Response) => {
        ...
        const post = await deps.variantsService.findById(String(req.params.id));
        if (post?.status !== "approved") {
            res.status(409).json({ message: "Variant must be approved before scheduling" });
        }
    })
